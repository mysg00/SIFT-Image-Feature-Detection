...
%base template used for concatenation
arrayHold = [0 0 0];
...
for i = 2: +1: 15
    ...
    for j = thresholdImageBoundary:+1:rowsEnd
        for k = thresholdImageBoundary:+1:colsEnd
            ...
            if (value > max(areaTop(:)) && value > max(areaBot(:)) && value > (thresholdDOG + max(areaCenter(1,:))) && value > (thresholdDOG + max(areaCenter(3,:))) && value > (thresholdDOG + max(areaCenter(:,1))) && value > (thresholdDOG + max(areaCenter(:,3))))
            ...
            elseif (value < min(areaTop(:)) && value < min(areaBot(:)) && value < (min(areaCenter(1,:)) - thresholdDOG) && value < (min(areaCenter(3,:)) - thresholdDOG) && value < (min(areaCenter(:,1)) - thresholdDOG) && value < (min(areaCenter(:,3)) - thresholdDOG))
                %if the center pixel satisfies the condition, then it
                %becomes a candidate point
                isCandidate(j,k) = 1;
                countCandidate = countCandidate + 1;
...
hold on;
colorbar
viscircles([arrayCol arrayRow], arrayRad ,'Color','r');
hold off;
